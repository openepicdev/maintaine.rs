...
2. Keep your CI pipelines safe
   ...
   - Each dependency on your CI pipeline (this includes applications / bots in your repo) is a possible vector of attack, each `run` field is a weak point.[^272]
3. Better and smaller pull requests produce better code
   ...
   - Get in the mud, explore the deep end of your contributor's code (You can actually fetch a remote pull request with `git fetch origin pull/$pr_num/head:$branch_name`!). Break your tests, read documentation for every single one of the added functions, see if functions could be removed, check if loops could be early-returned.
   - Don't forget to talk to your contributors about documentation!

4. Use automated tooling

   - This is a very simple step, don't guess about memory leaks, use a heap memory profiler (like heaptrack[^273]). Don't guess about memory safety, use a static
     code analyzer or a language that has memory safety built-in (like Rust). Don't guess about the origin of something, bisect it in your program (like with `git bisect`[^274]).
   - Know your system, the better you know the tools you're using the better code you'll produce and the faster you'll be able to iterate on a design. With this I don't mean learning a fancy-schmancy IDE or keyboard layout, but learning to make `perf`[^275] valuable, learning to read stack traces, learning to efficiently search throughout your documentation to find that edge case that has been bugging you out all week.
...
[^270]: https://github.com/rust-lang/rust-clippy
[^271]: https://rust-lang.org
[^272]: https://github.com/advisories/GHSA-8v8w-v8xg-79rf
[^273]: https://github.com/KDE/heaptrack
[^274]: https://git-scm.com/docs/git-bisect
[^275]: https://perfwiki.github.io/main/
